...
%% -----------------------第一阶段---------------------------
%% 模型数据
points = importdata("points_30min_102.txt");   % 所有集合中的坐标点，包括了原上下车点
% sets_index = importdata("merged_set_index.txt");
distances = importdata("distances_30min_102.txt");   % 距离矩阵
distances_km = distances/1000;

% 候选点集合，每个集合第一个元素为原点，其余元素为候选点
filename = 'merged_set_index_30min_102.txt';
...
% 原上车时间
pick_up_time = importdata("pickup_time_30min_102.txt")'; % 上车时间
